...
export interface FleetDefaults {
  ...
  distill?: { agent?: string; model?: string; runOn?: "orchestrator" | "worker" };
}
...
// Decide where a review or distill task should actually run.
//
// Rules, in priority order:
//   1. Explicit task.runOn → honored.
//   2. Explicit fleetDefaults.<kind>.runOn → honored. For kind='distill',
//      this falls through to fleetDefaults.review.runOn when distill
//      doesn't carry one (matches how agent/model fall through — distill
//      inherits review defaults unless explicitly overridden).
//   3. Any configured kind-specific agent or model (per-task OR in
//      fleetDefaults.<kind>.{agent,model}, with distill falling through
//      to review.{agent,model}) → worker lane. The configured values are
//      only honored on the worker lane; orchestrator-lane runs use the
//      host session's agent/model. Treating any explicit config as "use
//      worker" makes the configured values a hard contract instead of
//      "use them only if they happen to match the host" (which would
//      silently change behavior any time the host model changes).
//   4. Default → orchestrator lane for kind='review', worker lane for
//      kind='distill'. Distill defaults to worker because its job is
//      structured-output generation over potentially-large candidate
//      diffs; the host session is more valuable for interactive work.
//      Reviews default to orchestrator because they're often fast and
...
export function resolveTaskLane(
  ...
  const fleetRunOn =
    kind === "review"
      ? fleet.review?.runOn
      : (fleet.distill?.runOn ?? fleet.review?.runOn);
  if (fleetRunOn) {
  ...
  const kindModel =
    kind === "review"
      ? fleet.review?.model
      : (fleet.distill?.model ?? fleet.review?.model);
  const cfgAgent = task.agent ?? kindAgent;
  const cfgModel = task.model ?? kindModel;
  if (cfgAgent) {
  ...
  return { lane: kind === "distill" ? "worker" : "orchestrator", reason: "default" };
}

// Thin wrapper preserved for existing callers; delegates to
// resolveTaskLane with kind='review'.
...
export function parseFleetDefaultsFromObject(raw: unknown): FleetDefaults {
  ...
  if (r.distill && typeof r.distill === "object" && !Array.isArray(r.distill)) {
    ...
    if (typeof d.runOn === "string" && (d.runOn === "orchestrator" || d.runOn === "worker")) {
      bucket.runOn = d.runOn;
    }
    if (bucket.agent !== undefined || bucket.model !== undefined || bucket.runOn !== undefined) {
      fd.distill = bucket;
    }
